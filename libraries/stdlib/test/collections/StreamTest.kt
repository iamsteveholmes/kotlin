...
public class StreamTest {
    ...
    test fun flatMap() {
        val result = streamOf(1, 2).flatMap { (0..it).stream() }
        assertEquals(listOf(0, 1, 0, 1, 2), result.toList())
    }

    test fun flatMapOnEmpty() {
        val result = streamOf<Int>().flatMap { (0..it).stream() }
        assertEquals(listOf<Int>(), result.toList())
    }

    test fun flatMapWithEmptyItems() {
        val result = streamOf(1, 2, 4).flatMap { if (it == 2) streamOf<Int>() else (it - 1..it).stream() }
