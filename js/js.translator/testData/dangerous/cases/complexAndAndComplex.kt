...
fun box(): String {
    ...
    assertEquals(2, sideEffect, "true && true side effect")

    assertEquals(false, when { else -> { ++sideEffect; false }} && when { else -> { ++sideEffect; true }}, "true && false")
    assertEquals(3, sideEffect, "true && false side effect")

    assertEquals(false, when { else -> { ++sideEffect; true }} && when { else -> { ++sideEffect; false }}, "false && true")
    assertEquals(5, sideEffect, "false && true side effect")
