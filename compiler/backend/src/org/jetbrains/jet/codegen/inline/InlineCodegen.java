...
public class InlineCodegen implements CallGenerator {
    ...
    private InlineResult inlineCall(MethodNode node) {
        ...
        LabelOwner lowner = new LabelOwner() {

            final String functionName = codegen.getContext().getContextDescriptor().getName().asString();

            final boolean isLambda = CodegenBinding.isLocalFunOrLambda(codegen.getContext().getContextDescriptor()) &&
                               codegen.getContext().getContextDescriptor().getName().isSpecial();
        ...
        List<MethodInliner.FinnallyBlockInfo> infos = MethodInliner.processReturns(adapter, lowner, true, null);
        generateAndInsertFinallyBlocks(adapter, infos);

        adapter.accept(new InliningAdapter(codegen.v));
        return result;
    }
    ...
    public void generateAndInsertFinallyBlocks(MethodNode node, List<MethodInliner.FinnallyBlockInfo> infos) {
        if (codegen.hasFinallyBLocks()) {
            for (MethodInliner.FinnallyBlockInfo info : infos) {
                ...
                ExpressionCodegen finallyCodegen =
                        new ExpressionCodegen(finallyNode, codegen.getFrameMap(), codegen.getReturnType(),
                                              ...
                                              codegen.getBlockStackElements());

                finallyCodegen.generateFinallyBlocksIfNeeded(info.returnType);

                InlineCodegenUtil.insertNodeBefore(finallyNode, node, info.beforeIns);
            }
        }
    }
