...
public class MethodInliner {
    ...
    public InlineResult doInline(
        ...
        resultNode.accept(new InliningAdapter(adapter));
                ...
                return new Frame<SourceValue>(nLocals, nStack) {
                    ...
                    public void execute(
                        ...
                        if (insn.getOpcode() == Opcodes.RETURN) {
                            //there is exception on void non local return in analyzer
                            return;
                        }
    ...
    public static List<FinnallyBlockInfo> processReturns(@NotNull MethodNode node, @NotNull LabelOwner labelOwner, boolean remapReturn, Label endLabel) {
        if (!remapReturn) {
            return Collections.emptyList();
        }
        List<FinnallyBlockInfo> result = new ArrayList<FinnallyBlockInfo>();
        ...
        while (insnNode != null) {
            if (InlineCodegenUtil.isReturnOpcode(insnNode.getOpcode())) {
                ...
                result.add(new FinnallyBlockInfo(isLocalReturn ? insnNode : insnNode.getPrevious(), getReturnType(insnNode.getOpcode())));
            }
    ...
    public static class FinnallyBlockInfo {

        final AbstractInsnNode beforeIns;

        final Type returnType;

        public FinnallyBlockInfo(AbstractInsnNode beforeIns, Type returnType) {
